"""接口调试经压测 Worker 代发：等待回传结果（优先 Redis，兼容单机内存）。"""
...
_RESULT_KEY = "perf:api_debug_result:{request_id}"
_TTL_SECONDS = 600
...
# Redis 可用性短缓存，避免不可达时每次 ping 拖死等待循环
...
async def _redis():
    ...
    try:
        ...
        _redis_ok = False
        _redis_checked_at = now
        logger.debug("api_debug_bridge: Redis 不可用: %s", exc)
        return None
...
async def complete(request_id: str, result: dict[str, Any]) -> bool:
    ...
    if r is not None:
        try:
            ...
            logger.error("api_debug_bridge: Redis SET 失败: %s", exc)
...
async def _read_redis(request_id: str) -> Optional[dict[str, Any]]:
    ...
    try:
        ...
        logger.warning("api_debug_bridge: Redis GET 失败: %s", exc)
        return None
