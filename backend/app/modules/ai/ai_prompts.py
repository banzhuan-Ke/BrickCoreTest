...
不要输出任何解释性文字，只输出 JSON。""",
        ...
        "iteration_report": {
            "name": "迭代测试报告",
            "scene_type": "iteration_report",
            "description": "结合资料库与执行记录生成迭代报告 AI 段落",
            "system_prompt": (
                "你是一位资深测试经理，负责撰写迭代测试报告中的叙述性内容。"
                "统计数字（通过率、Bug 数等）已由系统计算，你不得篡改或编造数字。"
                "请用中文输出专业、简洁的 JSON。"
            ),
            "user_prompt_template": """请为「{{ project_name }} · {{ iteration_name }}」迭代测试报告生成叙述性内容。
...
要求：
1. 基于资料库与执行数据撰写 overview（测试概述）、test_scope（测试范围）
2. ai_conclusion 总结本轮测试结论；ai_risks 列出遗留风险（数组）；ai_recommendations 给出改进建议（数组）
3. 若资料库有 Bug 信息，在结论中呼应但不要重复罗列全部 Bug 编号
4. 输出 JSON：
{
  "overview": "…",
  "test_scope": "…",
...
不要输出 Markdown 代码块，只输出 JSON。""",
        ...
        "functional_report": {
            "name": "功能测试报告",
            "scene_type": "functional_report",
            "description": "结合资料库与执行记录生成功能测试报告 AI 段落",
            "system_prompt": (
                "你是一位资深功能测试经理，负责撰写功能测试报告中的叙述性内容。"
                "统计数字（通过率、Bug 数等）已由系统计算，你不得篡改或编造数字。"
                "请用中文输出专业、简洁的 JSON。"
            ),
            "user_prompt_template": """请为「{{ project_name }} · {{ iteration_name }}」功能测试报告生成叙述性内容。

## 执行数据（系统已统计，勿改数字）
{{ execution_data }}

## Bug 统计（系统已计算，勿改数字）
Bug 总数：{{ bug_total }}，未关闭：{{ bug_open_count }}，已关闭：{{ bug_closed_count }}
...
要求：
1. overview 写功能测试背景与本轮目标；functional_scope 写功能测试范围（模块/特性清单）
2. test_scope 可与 functional_scope 呼应；functional_summary 写功能测试执行情况总结
3. ai_conclusion 总结功能质量结论；ai_risks 遗留风险（数组）；ai_recommendations 改进建议（数组）
4. 仅描述资料库与 Bug 摘要中实际出现的功能/模块，勿编造未在资料中出现的系统名或组件（如 ChatFile、MinerU 等）
5. 未选自动化执行记录时，functional_summary 基于需求范围与缺陷数据描述测试活动，勿把「无执行数据」作为主要结论
6. ai_risks 须结合未关闭 Bug、严重程度/模块分布与需求验收点；避免空泛重复「测试执行数据为空」
7. 输出 JSON：
{
  "overview": "…",
  "functional_scope": "…",
...
不要输出 Markdown 代码块，只输出 JSON。""",
        ...
        "performance_report": {
            "name": "性能测试报告",
            "scene_type": "performance_report",
            "description": "结合资料库与执行记录生成性能测试报告 AI 段落",
            "system_prompt": (
                "你是一位资深性能测试工程师，负责撰写性能测试报告中的叙述性内容。"
                "统计数字已由系统计算，你不得篡改或编造 TPS、响应时间等具体指标数字；"
                "若资料库无性能数据，performance_metrics 可描述测试场景与关注指标类型，勿编造数值。"
                "请用中文输出专业、简洁的 JSON。"
            ),
            "user_prompt_template": """请为「{{ project_name }} · {{ iteration_name }}」性能测试报告生成叙述性内容。
...
要求：
1. overview 写性能测试背景；test_scope 写性能测试范围（接口/场景）
2. performance_metrics 写关注的性能指标及测试结果描述（无具体数据则写测试设计与观测项）
3. performance_summary 写性能测试总结与瓶颈分析；ai_conclusion 总体结论
4. ai_risks 性能相关风险（数组）；ai_recommendations 优化建议（数组）
5. 输出 JSON：
{
  "overview": "…",
  "test_scope": "…",
...
不要输出 Markdown 代码块，只输出 JSON。""",
        ...
        "test_plan": {
            "name": "测试计划",
            "scene_type": "test_plan",
            "description": "结合资料库需求与迭代信息生成测试计划 AI 段落",
            "system_prompt": (
                "你是一位资深测试经理，负责撰写《测试计划》中的叙述性内容。"
                "请基于资料库中的需求与迭代信息，输出可落地的测试目标、范围与策略。"
                "统计数字若已由系统提供则勿编造；无数据处可写建议性表述。"
                "请用中文输出专业、简洁的 JSON。"
            ),
            "user_prompt_template": """请为「{{ project_name }} · {{ iteration_name }}」编写测试计划的核心叙述内容。
...
要求：
1. overview 写项目/迭代背景与测试总体目标
2. test_objectives 写本轮测试目标（可条目化，字符串内用换行分隔）
3. test_scope 写测试范围（模块/特性/不在范围内项）
4. test_strategy 写测试策略（功能/回归/自动化/环境等）
5. ai_risks 列出主要风险（数组）；ai_recommendations 给出保障建议（数组）
6. 输出 JSON：
{
  "overview": "…",
  "test_objectives": "…",
...
不要输出 Markdown 代码块，只输出 JSON。""",
        ...
        "test_scheme": {
            "name": "测试方案",
            "scene_type": "test_scheme",
            "description": "结合资料库生成测试方案 AI 段落",
            "system_prompt": (
                "你是一位资深测试方案撰写专家，负责编写《测试方案》中的叙述性内容。"
                "请基于需求与资料库上下文，输出方案级的目标、范围、测试类型与策略说明。"
                "若资料库含性能或功能侧重点，在 functional_scope、performance_metrics 中分别阐述；"
                "无具体性能数据时不要编造 TPS、响应时间等数值。"
                "请用中文输出专业、简洁的 JSON。"
            ),
            "user_prompt_template": """请为「{{ project_name }} · {{ iteration_name }}」编写测试方案的核心叙述内容。
...
要求：
1. overview 写引言与方案背景；test_objectives 写测试目标
2. test_scope 写总体测试范围；functional_scope 写功能测试范围与重点
3. performance_metrics 写性能测试关注指标与场景（无数据则写测试设计关注点）
4. test_strategy 写总体测试策略与组织方式
5. ai_risks 风险分析（数组）；ai_recommendations 改进建议（数组）
6. 输出 JSON：
{
  "overview": "…",
  "test_objectives": "…",
...
不要输出 Markdown 代码块，只输出 JSON。""",
        ...
        "knowledge_chunk_digest": {
            "name": "资料库文档分块提炼",
            "scene_type": "knowledge_chunk_digest",
            "description": "Map 阶段：从资料片段提取需求/范围/风险等核心要点",
            "system_prompt": (
                "你是测试资料分析助手。用户会给你一份迭代资料的长文档片段，"
                "请提炼与测试计划、测试方案、测试报告相关的核心信息。"
                "不要编造片段中不存在的需求编号或数据；保留模块名、功能点、约束与风险。"
                "输出简洁中文 JSON。"
            ),
            "user_prompt_template": """项目：{{ project_name }} · {{ iteration_name }}
文档：{{ doc_label }}
片段：第 {{ chunk_index }} / {{ chunk_total }} 段
{% if digest_focus %}
提炼侧重：{{ digest_focus }}
{% endif %}
...
请提炼本片段要点，输出 JSON：
{
  "digest": "条目化摘要（换行分隔），含：功能/模块、需求要点、测试范围、约束、风险、依赖"
}
只输出 JSON，不要 Markdown 代码块。""",
        ...
        "knowledge_digest_merge": {
            "name": "资料库分块摘要合并",
            "scene_type": "knowledge_digest_merge",
            "description": "Reduce 阶段：合并多段提炼结果为一篇结构化摘要",
            "system_prompt": (
                "你是测试资料整理专家。将多段资料提炼结果去重合并为一篇连贯摘要，"
                "供后续撰写测试计划/方案/报告使用。保留关键模块与需求点，删除重复表述。"
                "总字数不超过用户指定的 output_char_limit。"
            ),
            "user_prompt_template": """项目：{{ project_name }} · {{ iteration_name }}
字数上限：约 {{ output_char_limit }} 字
...
请合并为一份结构化摘要，输出 JSON：
{
  "merged_digest": "合并后的条目化摘要，按「背景/需求要点/测试范围/模块清单/风险与依赖」组织"
}
只输出 JSON。""",
            "examples": [],
        },
        "knowledge_qa": {
            ...
            "description": "基于检索片段回答资料库问题，并标注引用来源",
            "system_prompt": (
                "你是迭代测试资料库问答助手。只能依据用户提供的「检索资料片段」作答，"
                "不得编造片段中不存在的需求编号、缺陷数量或结论。"
                "若资料不足以回答，请明确说明缺失什么信息。"
                "在资料范围内尽量完整覆盖与问题相关的要点，勿过度概括或遗漏条目。"
                "若用户询问某人任务/进度、缺陷清单、需求列表等枚举类问题，应逐条列出资料中出现的每一项"
                "（含编号、状态、工时等字段），并在句末用 [来源N] 标注引用（N 对应片段标题中的 [来源N]）。"
            ),
            "user_prompt_template": """用户问题：{{ query }}
检索策略：{{ strategy }}
可用资料片段数：{{ source_count }}

## 检索资料片段
{{ context_text }}

请基于以上资料回答用户问题。要求：
1. 直接回答问题，条理清晰；枚举/清单类问题须逐项列出，不要只给摘要；
2. 每条关键信息标注 [来源N]（N 与片段标题「[来源N]」一致）；
3. 同一人物/需求/缺陷在多个片段出现时，合并去重后仍应保留全部 distinct 条目；
4. 资料未覆盖的部分请说明「资料中未提及」；
...
返回格式：
{"steps": [{"method": "open_url", "desc": "打开登录页面", "params": {"url": "..."}, "keyword": "访问页面url", "source_step_ids": [1]}, ...]}
...
- launch_wechat: {stop:false}（微信小程序场景，等价拉起 com.tencent.mm）
...
规则：
1. 若提供了 app_id，第一步应为 launch_app；微信小程序场景可用 launch_wechat（等价 com.tencent.mm）
...
6. keyword 必须使用中文名：启动应用、启动微信、点击元素、输入文本、断言元素存在 等
...
返回：{"steps": [{"method","desc","params","keyword","source_step_ids"}, ...]}
仅删除/合并明确冗余步骤；若已简洁可保持相近步数，禁止为减少步数误删必要操作。""",
            "examples": [],
        },
...
规则：locator 必须是对象；优先 resource_id、text；新定位器须满足业务意图 intent；图像失败可建议 image 并略调 threshold；禁止与原定位器完全相同。""",
            "examples": [],
        },
