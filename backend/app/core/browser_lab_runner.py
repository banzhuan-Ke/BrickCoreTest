...
from app.core.browser_use_llm import browser_use_supports_vision, build_browser_use_llm
...
async def run_browser_lab_task(task_id: int, username: str = "") -> None:
    ...
    async with _run_semaphore:
        ...
        try:
            from app.core.browser_lab_gif_patch import apply_browser_use_gif_cjk_patch
        ...
        if fatal_exc is not None and history is None:
            ...
            if _should_stop(task_id):
                ...
                task.status = "failed"
                task.error_message = str(fatal_exc)[:4000]
            task.finished_at = datetime.now(timezone.utc)
            await task.save()
            await _append_step_event(
                task_id,
        ...
        if _should_stop(task_id):
            ...
            task.status = "failed"
            task.error_message = (error_msg or summary or "执行失败")[:4000]
