...
async def merge_exec_variables(
    ...
    return {**proj_vars, **(env.global_vars or {}), **(accumulated_vars or {})}
    ...
    try:
        for case_id in case_ids:
            result = await runner(
                case_id,
                env.id,
                suite_record.id if suite_record else None,
                username,
                accumulated_vars,
