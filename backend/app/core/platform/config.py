...
RUNNER_ENGINE_VERSION = os.getenv("RUNNER_ENGINE_VERSION", "1.0.3")
# 定位策略打分（recorder_locator_rank）所需最低引擎版本
RUNNER_ENGINE_VERSION_MIN = os.getenv("RUNNER_ENGINE_VERSION_MIN", "1.0.3")
RUNNER_CLIENT_VERSION_MIN = os.getenv("RUNNER_CLIENT_VERSION_MIN", "1.3.8")
RUNNER_CLIENT_VERSION_LATEST = os.getenv("RUNNER_CLIENT_VERSION_LATEST", "1.4.5")
# 平台产品版本（页脚、/runner/version 展示，与 Runner 客户端版本独立）
PLATFORM_VERSION = os.getenv("PLATFORM_VERSION", "1.4.0")
...
KNOWLEDGE_FULLTEXT_MAX_CHARS = int(os.getenv('KNOWLEDGE_FULLTEXT_MAX_CHARS', '32000'))
KNOWLEDGE_MAPREDUCE_ENABLED = os.getenv('KNOWLEDGE_MAPREDUCE_ENABLED', '1').strip().lower() in (
    '1', 'true', 'yes', 'on',
)
KNOWLEDGE_MAPREDUCE_CHUNK_CHARS = int(os.getenv('KNOWLEDGE_MAPREDUCE_CHUNK_CHARS', '10000'))
KNOWLEDGE_MAPREDUCE_MAX_CHUNKS = int(os.getenv('KNOWLEDGE_MAPREDUCE_MAX_CHUNKS', '24'))
KNOWLEDGE_MAPREDUCE_CONCURRENCY = int(os.getenv('KNOWLEDGE_MAPREDUCE_CONCURRENCY', '3'))

# 资料库向量 Embedding（Phase 4a+；默认关闭，项目级可再开）
KNOWLEDGE_VECTOR_EMBED_ENABLED = os.getenv('KNOWLEDGE_VECTOR_EMBED_ENABLED', '0').strip().lower() in (
    '1', 'true', 'yes', 'on',
)
KNOWLEDGE_EMBED_DEFAULT_PROVIDER = os.getenv('KNOWLEDGE_EMBED_DEFAULT_PROVIDER', 'qwen').strip().lower()
KNOWLEDGE_EMBED_DEFAULT_MODEL = os.getenv('KNOWLEDGE_EMBED_DEFAULT_MODEL', 'text-embedding-v3').strip()
