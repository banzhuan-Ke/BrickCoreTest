...
APP_VALID_METHODS = {
    "launch_app", "launch_wechat", "terminate_app", "clear_app", "install_app", "uninstall_app",
    "click_element", "double_click_element", "long_press_element",
    "input_text", "clear_text", "get_text",
    "wait_element", "wait_gone", "wait_for_time",
    "swipe", "scroll_to",
    "assert_exists", "assert_not_exists", "assert_image_exists", "assert_image_not_exists",
    "assert_text", "assert_text_contains", "assert_device_metric",
    "extract_text",
...
APP_DEFAULT_PARAMS = {
    ...
    "assert_text": {"timeout": 10},
    "assert_text_contains": {"timeout": 10},
    "assert_device_metric": {"operator": "lt", "agg": "current", "expected": 80},
    "wait_for_time": {"seconds": 2},
    "swipe": {"direction": "up"},
    "screenshot": {"name": "screenshot"},
    "launch_app": {"stop": False},
    "launch_wechat": {"stop": False},
...
METHOD_TO_KEYWORD = {
    ...
    "assert_image_exists": "断言图像存在",
    "assert_image_not_exists": "断言图像不存在",
    "assert_text": "断言文本等于",
    "assert_text_contains": "断言文本包含",
    "assert_device_metric": "断言设备性能",
    "extract_text": "提取元素文本",
    "press_key": "按键",
    "press_back": "返回",
    "open_url": "打开链接",
