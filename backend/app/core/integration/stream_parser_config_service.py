...
from app.modules.stream_phase.registry import get_parser, list_parsers, parse_stream_lines
...
async def test_stream_parser_config(
    *,
    ...
    samples = raw.get("match_view_samples") if isinstance(raw, dict) else None
    if samples:
        out["match_view_samples"] = samples
    return out
