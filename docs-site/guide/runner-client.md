...
| **客户端检查更新** | 已登录客户端可对比版本；**优先加密分层增量**一键覆盖重启，否则下载完整 zip / 打开网盘外链 |
...
> **单实例（v1.5）**：同一设备（`DEVICE_ID`）本机只允许一个 UI 执行器。若已上线或另有 `python main.py` 在跑，再次上线会提示「已有执行器在运行」；请先下线或结束旧进程。可避免两个消费者抢同一 MQ 队列导致套件时好时坏。

## 界面说明

| 区域 | 说明 |
|------|------|
| 连接状态 | 平台 API / 消息队列 / Redis / Web·App / 执行机 指示灯 |
| 本机能力 | **复选**：Web、App、接口代发、压测（选啥用啥）。接口代发与压测共用同一执行机进程和项目 |
| 登录 / 上线 / 下线 | 登录平台账号；上线启动对应子进程；下线自动注销设备/执行机 |
| 检查更新 | 对比 `/runner/version`；有 `patches` 时一键增量，否则下载完整包（需已登录） |
| 管理… | 增删改服务器环境列表 |
...
3. **实时画面**：设备管理 → 实时画面（设备须在线）。**跑 Web 用例时**推的是自动化浏览器当前页（不再是整桌面其它窗口）；空闲仍可能是桌面。若长时间「等待执行机画面」而日志仍正常：新版 Runner 会自动重试截屏；仍黑屏可重启执行器后再看。可用环境变量 `RUNNER_PAGE_SCREEN_STREAM=0` 关闭页流、回退桌面截图。
4. **定时任务**：计划触发时 MQ 下发到已上线设备

### 性能压测（必需 Worker）

1. 登录 → 在 **本机能力** 勾选 **压测**（可同时勾选 **接口代发**；二者共用同一执行机进程和项目）
2. **所属项目** 选择与平台顶部一致的项目 → **上线**
3. 在 **性能测试 → 执行机** 确认节点在线
4. 在场景列表 **执行**（平台已一律派发在线 Worker，无 Worker 时无法启动）
5. 压测进行中在客户端 **当前会话日志** 查看秒级 QPS/RT；完整日志见客户端安装目录

> 压测施压由执行机提供，平台不再本机直跑。任务与上报字段见 [压测 Worker 协议](./perf-worker-protocol.md)。

**仅压测、不要整包浏览器时**：可改用精简包 **BrickCorePerf**（解压 + `start-perf`），与完整客户端二选一；见 [性能测试 · 执行机](./perf-testing.md#执行机worker)、包内 `README-PERF.md`。
...
- **1.3.15**：修复 hover 步骤将 `timeout` 误当作悬停后等待导致每次多 sleep 的问题；`timeout` 仅作 Playwright 定位超时，悬停后停留用 `wait_time`（默认 500ms）
...
| 压测无日志 | 确认已勾选 **压测** 并上线；平台触发压测后看会话日志（前缀 `[执行机]`）或 `runner/logs/perf_worker.log` |
...
| 同环境同 Runner，套件一成功一失败 | 几乎总是 **重复 `main.py` 进程** 抢 MQ；新版会拦截重复上线，仍建议 PowerShell 自查见 [Runner 排查指南](./runner-troubleshooting.md) |
| 上线提示「本机已有该设备的 UI 执行器在运行」 | 正常：单实例互斥已生效；客户端下线，或结束旧 `main.py` / 旧客户端后再上线 |
