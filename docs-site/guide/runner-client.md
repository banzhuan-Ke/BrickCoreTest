...
| 方式 | 说明 |
|------|------|
| **网盘下载（推荐）** | [百度网盘](https://pan.baidu.com/s/1Nx2fkPAUi7htJKZAxp1paw?pwd=ye6b)（提取码 `ye6b`）；或见 [README 执行器章节](../../README.md#执行器下载windows--约-800mb) |
...
- **1.3.14**（当前推荐）：打开客户端默认「仅 UI 执行器」；压测引擎同步流式持续/梯度压测与 CSV 问题参数化；操作日志识别压测 Worker 节点
