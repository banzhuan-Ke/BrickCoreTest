<template>
  <div class="global-headers-editor">
    ...
    <div class="quick-hints">
      ...
      <span class="hint-text">值支持 <code v-pre>${{变量名}}</code>；环境级 Header 会覆盖项目同名字段</span>
    </div>
...
function loadFromModel(val) {
  syncing.value = true
  headerList.value = normalizeHeaderList(val)
  syncing.value = false
}
...
function addRow() {
  headerList.value.push({ key: '', value: '', description: '', enabled: true })
}
...
function exportJson() {
  ...
  a.href = url
  a.download = 'default-headers.json'
  a.click()
