<template>
  <el-collapse v-model="activeNames" class="case-used-vars-panel">
    <el-collapse-item name="vars">
      ...
      <p class="panel-hint">
        自动扫描步骤中的 <code v-pre>${{变量名}}</code>、<code v-pre>${{df:标签}}</code>、<code v-pre>${{dt:...}}</code> 等占位符；片段内未展开的引用需在片段步骤中查看。
      </p>
      ...
      <el-empty v-else description="当前用例步骤中未检测到变量占位符" :image-size="56" />
    </el-collapse-item>
  </el-collapse>
</template>
...
import { extractCaseVarRefs } from '@/utils/caseVarRefs'
...
const rows = computed(() => extractCaseVarRefs(props.steps))
</script>
