<template>
  <div>
    <div class="filter-bar">
      <el-input v-model="filters.keyword" placeholder="关键词" clearable style="width: 180px;" @keyup.enter="loadList" />
      <el-select v-model="filters.status" placeholder="状态" clearable style="width: 100px;" @change="loadList">
        <el-option label="草稿" value="draft" />
        <el-option label="已确认" value="confirmed" />
      </el-select>
      <el-select v-model="filters.test_type" placeholder="类型" clearable style="width: 100px;" @change="loadList">
        <el-option v-for="t in ['正向','异常','边界','权限','状态','兼容','性能','API','其他']" :key="t" :label="t" :value="t" />
      </el-select>
      <el-select v-model="filters.priority" placeholder="优先级" clearable style="width: 90px;" @change="loadList">
...
const page = ref(1)
const pageSize = 50
const filters = reactive({ keyword: '', status: '', test_type: '', priority: '' })
...
const loadList = async () => {
  ...
  try {
    ...
    const res = await aiTestAnalysisApi.listProjectTestPoints(params)
...
const goRequirement = (row) => {
  router.push({
    ...
    query: { tab: 'points', pointId: row.id }
  })
}
