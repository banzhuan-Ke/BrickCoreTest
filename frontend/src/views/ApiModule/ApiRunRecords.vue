  ...
  <div class="run-records">
    ...
    >
      ...
      <template v-for="col in activeColumns" :key="col.key">
        ...
        <el-table-column
          v-else-if="col.key === 'trigger_type'"
          prop="trigger_type"
          label="触发方式"
          :width="col.width"
        >
        ...
        <el-table-column
          v-else-if="col.key === 'http_duration'"
          prop="http_duration"
          label="接口总耗时(ms)"
          :width="col.width"
        >
      ...
      <el-table-column label="操作" width="320" fixed="right">
        <template #default="{ row }">
          <div class="table-row-actions">
            ...
            <el-button
              v-if="!['running', '执行中'].includes(row.status) && (row.suite_id || row.plan_id)"
              link
              type="warning"
...
const {
  ...
  setPickerOrder,
  resetColumns
} = useTableColumns('api.run_records')
...
const handleRerun = async (row) => {
  ...
  try {
    ...
    if (row.record_type === 'plan' && row.plan_id) {
      ...
      ElMessage.success('套件已重新提交执行')
    } else {
      ElMessage.warning('无法识别执行目标')
      return
    }
    fetchRecords()
  } catch (e) {
