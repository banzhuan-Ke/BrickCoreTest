    ...
    >
      ...
      <el-table-column prop="status" label="状态" width="110">
        <template #default="{ row }">
          <el-tag :type="suiteStatusTag(row.status).type">{{ suiteStatusTag(row.status).text }}</el-tag>
        </template>
      </el-table-column>
...
const props = defineProps({
  suiteId: { type: Number, required: true },
...
function suiteStatusTag(status) {
...
async function loadList() {
  if (!props.suiteId || !props.projectId) return
  loading.value = true
  try {
    const res = await appRecordApi.listSuites({
...
async function stopRun(recordId) {
  try {
    await ElMessageBox.confirm('确定停止当前套件执行吗？', '停止执行', { type: 'warning' })
...
async function deleteRecord(recordId) {
  try {
    await ElMessageBox.confirm('删除后该套件执行记录及下属用例记录将从列表中隐藏，确定继续吗？', '删除记录', { type: 'warning' })
    await appRecordApi.deleteSuite(recordId)
...
watch(() => [props.suiteId, props.projectId], loadList, { immediate: true })
</script>
